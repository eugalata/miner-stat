  ...
  $header =
    "<thead>
    	<tr>
            ...
            <th scope='col' class='rounded-q1'>Desired</th>
...
function process_host_disp($desmhash, $summary_data_array, $dev_data_array)
{
  ...
  if ($summary_data_array != null)
  {
	...
	$row = "
      <td $thisdevcol data-label=\"Devices\">$activedevs/$devs</td>
      <td $tempcol data-label=\"Temperature\">$max_temp&deg;C</td>
      <td data-label=\"Desired MH/s\">$desmhash</td>
...
function get_host_summary($host_data)
{
  ...
  $host_row = "<tbody><tr>
    <td><table border=0><tr>
      <td data-label=\"Host\"><a href=\"edithost.php?id=$hostid\"><span class=\"button\">Edit</span></a></td>
...
function process_dev_disp($gpu_data_array, $edit=false)
{
  ...
  if (isset($gpu_data_array['GPU']))
  {
    if ($privileged)
    {
      ...
      $DEV_cell =
      "<table border=0><tr>
        <td><a href='editdev.php?id=".$id."&dev=".$gpu_data_array['GPU']."&type=GPU'><span class=\"button\">Edit</span></a></td>
...
function create_totals()
{
    ...
    "<thead>
    	<tr>
        	<th scope='col' class='bottomleft rounded-company'>".$data_totals['hosts']." Hosts</th>
            ...
            <th id=\"Speed\" scope='col' class='rounded-q1'>".$data_totals['fivesmhash']."</th>
